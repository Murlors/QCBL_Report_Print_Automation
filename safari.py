...
class Qcbl:
    ...
    def login(self, username, password):
        ...
        self.wait.until_not(method=EC.title_contains("匿名用户"), message="超时啦")
        self.cookies = self.driver.get_cookies()
        self.get_stu_id()
        self.get_options_pdfkit()

    def get_stu_id(self):
        self.driver.find_element(By.PARTIAL_LINK_TEXT, "%s" % self.username).click()
    ...
    def by_volume(self, course_id):
        ...
        for t in volume_list:
            ...
            for row in rows:  # 逐行获取
                ...
                if pp > 0: dictionary_problem.update({
                    int(datalist_problem[pp][1]):
                        self.driver.find_element(By.XPATH, '//*[@id="problemStatus"]/table/tbody/tr[%d]/td[3]/a' % pp)
                            .get_attribute('href')})
                pp += 1
    ...
    def run(self):
        while True:
            ...
            if event == '_submit_':
                ...
                sg.popup('打印结束啦，可以退出了(也可以继续打印)', font=self.font, keep_on_top=True, )
